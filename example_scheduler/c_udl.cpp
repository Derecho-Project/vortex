...
class TaskC_OCDPO : public OffCriticalDataPathObserver {
public:
	void operator()(const derecho::node_id_t sender,
			...
			if(!globals) [[unlikely]] {
				...
				globals->my_task_id = globals->join_service->dag().find_task_id_by_path("/C").value_or(2);

				spdlog::info("[c_udl] initialized");
			}
			...
			spdlog::info("[c_udl] recv job={} from task={} msg='{}'",
			...
			for (const auto downstream_task : globals->join_service->dag().find_task(0, globals->my_task_id)->downstream) {
				...
				spdlog::info("[c_udl] emit job={} -> task={} msg='{}'",
	...
	static void initialize() {
		if(!ocdpo_ptr) {
			ocdpo_ptr = std::make_shared<TaskC_OCDPO>();
		}
	}
